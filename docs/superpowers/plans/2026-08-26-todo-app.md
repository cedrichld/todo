...
.chip.empty { display: none; }  /* not just hidden: an empty button still joins baseline alignment and inflates the row */
.row:hover .chip.empty, .row:focus-within .chip.empty { display: inline-block; }
...
.empty-state { color: var(--muted); text-align: center; margin-top: 3rem; }
